from flask import current_app
from flask_login import UserMixin

class User(UserMixin):
    def __init__(self, username, password, number_of_questions_added = 0, number_of_likes = 0, number_of_dislikes = 0):
        self.username = username
        self.password = password
        self.number_of_questions_added = number_of_questions_added
        self.number_of_likes = number_of_likes
        self.number_of_dislikes = number_of_dislikes
        self.active = True
        self.is_admin = False

    def get_id(self):
        return self.username

    @property
    def is_active(self):
        return self.active

def get_user(user_id):
    """
    password = current_app.config["PASSWORDS"].get(user_id)
    user = User(user_id, password) if password else None
    if user is not None:
        user.is_admin = user.username in current_app.config["ADMIN_USERS"]
    """
    db = current_app.config["db"]
    return db.get_user(user_id)